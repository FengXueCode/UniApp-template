<template>
  <div class="main">
    <up-navbar title="新增船舶" :autoBack="true"></up-navbar>
    ...
    <div class="content">
      <div class="user-info">
        ...
        <up-form :model="formShipData" labelAlign="right" ref="formShip" :rules="rules">
          <up-form-item label="姓名" prop="shipOwner" labelWidth="80">
            <up-input placeholder="请输入姓名" type="text" maxlength="10" border="bottom"
              v-model="formShipData.shipOwner"></up-input>
          </up-form-item>

          <up-form-item label="联系方式" prop="shipOwnerContact" labelWidth="80">
            <up-input placeholder="请输入联系方式" type="text" maxlength="11" border="bottom"
              v-model="formShipData.shipOwnerContact"></up-input>
          </up-form-item>
          <up-form-item label="身份证号" prop="shipOwnershipIdentificationNo" labelWidth="80">
            <up-input placeholder="请输入身份证号" type="idcard" maxlength="18" border="bottom"
              v-model="formShipData.shipOwnershipIdentificationNo"></up-input>
          </up-form-item>

          <div class="title">船舶信息登记</div>
          <up-form-item label="船舶名称" prop="shipName" labelWidth="80">
            <up-input placeholder="请输入船舶名称" type="text" maxlength="10" border="bottom"
              v-model="formShipData.shipName"></up-input>
          </up-form-item>
          <up-form-item label="AIS船名" prop="ais" labelWidth="80">
            <up-input placeholder="请输入AIS船名" type="text" maxlength="10" border="bottom"
              v-model="formShipData.ais"></up-input>
          </up-form-item>
          <up-form-item label="船牌" prop="shipPlateNo" labelWidth="80">
            <up-input placeholder="请输入船牌" type="text" maxlength="10" border="bottom"
              v-model="formShipData.shipPlateNo"></up-input>
          </up-form-item>
          <up-form-item label="船籍港" prop="harbourNo" labelWidth="80">
            <div class="select" :class="{ 'no-input': formShipData.harbourNo == '' }" @click="showHarbour = true">
              <div class="text">
            ...
            <up-picker @cancel="showHarbour = false" confirmColor="#07C160" :show="showHarbour" :columns="[harhourList]"
              keyName="desc" @confirm="sureHarbour" :defaultIndex="checkHarbour"></up-picker>
          </up-form-item>
          <up-form-item label="船舶类型" prop="shipType" labelWidth="80">
            <div class="select" :class="{ 'no-input': formShipData.shipType == '' }" @click="showShipType = true">
              <div class="text">
                {{
                  formShipData.shipType == "" ? "请选择船舶类型" : shipType.label
                }}
              </div>
              <up-icon name="arrow-right"></up-icon>
            </div>

            <up-picker @cancel="showShipType = false" confirmColor="#07C160" :show="showShipType"
              :columns="[shipTypeList]" keyName="label" @confirm="sureShipType" :defaultIndex="checkType"></up-picker>
          </up-form-item>
          <up-form-item label="归属行政区" prop="region" labelWidth="80">
            <picker class="pciker" :value="checkRegion" mode="region" @change="changeRegion">
              <div class="select" :class="{ 'no-input': formShipData.region === '' }" @click="showRegion = true">
          ...
          <up-form-item label="航线类型" prop="routeType" labelWidth="80">
            <div class="select" :class="{ 'no-input': formShipData.routeType === '' }" @click="showRouteType = true">
              <div class="text">
                {{
            ...
            <up-picker @cancel="showRouteType = false" confirmColor="#07C160" :show="showRouteType"
              :columns="[routeTypeList]" keyName="label" @confirm="sureRouteType" :defaultIndex="checkRoute"></up-picker>
          </up-form-item>
          ...
          <up-form-item label="船舶证件照" prop="shipAuditData" labelWidth="80">
            <up-upload :fileList="fileList2" @afterRead="afterRead1" @delete="deletePic1" name="1" multiple
              :maxCount="1"></up-upload>
          </up-form-item>
        </up-form>
      </div>
    ...
    <div class="bottom-btn">
      <div class="btn-full" @click="save">
        <div class="item submit">保存</div>
      </div>
    </div>
  </div>
</template>
...
const checkRoute = ref([0])
function sureRouteType(e) {
  checkRoute.value = e.indexs;
...
const checkHarbour = ref([])
function sureHarbour(e) {
  checkHarbour.value = e.indexs
    ...
    success(res) {
      if (res.confirm) {
        formShip.value.validate().then((valid) => {
          if (valid) {
            request(
              "/fisherApi/wechatMiniProgram/addShip",
              [{ ...formShipData.value }],
              "POST",
            )
              .then((res) => {
