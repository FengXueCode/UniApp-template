<template>
  <div class="main">
    <up-navbar title="船舶详情" :autoBack="true"></up-navbar>
        ...
        >
          ...
          <up-form-item label="船牌" prop="shipPlateNo" labelWidth="80">
            <up-input
              ...
              maxlength="10"
              border="bottom"
              v-model="formShipData.shipPlateNo"
            ></up-input>
          </up-form-item>
<up-form-item label="船籍港" prop="harbourNo" labelWidth="80">
          ...
          <up-form-item label="航线类型" prop="routeType" labelWidth="80">
            ...
            <up-picker
              ...
              :defaultIndex="checkRoute"
            ></up-picker>
          </up-form-item>         <up-form-item label="船舶照片" prop="shipPhotos" labelWidth="80">
            <up-upload
              :fileList="fileList1"
              @afterRead="afterRead"
              @delete="deletePic"
    ...
    <div class="bottom-btn">
      ...
      <div class="btn-full" @click="save">
        <div class="item submit" >修改信息</div>
      </div>
    </div>
  </div>
</template>
...
onLoad((val: any) => {
  ...
  checkHarbour.value[0] = harhourList.value.findIndex(
    (element) => element.value == formShipData.value.harbourNo
  );
    shipType.value = shipTypeList.find(
      (element) => element.value == formShipData.value.shipType
    );
    checkType.value[0] = shipTypeList.findIndex(
      element=>element.value == formShipData.value.shipType
    )
    ...
    checkRoute.value[0] = routeTypeList.findIndex(
      (element) => element.value == formShipData.value.routeType,
    );
    ...
    fileList1.value.push({
      ...
      type: "image",
      thumb: "",
      url:import.meta.env.VITE_BASE_URL+'/fisherApi/storage/preview?filePath='+ shipPhotos.filePath,
    });
    ...
    fileList2.value.push({
      ...
      url:import.meta.env.VITE_BASE_URL+
        '/fisherApi/storage/preview?filePath='+ shipAuditData.filePath,
    });
  }, 500);
});
onMounted(() => {
    ...
    success(res) {
      if (res.confirm) {
        formShip.value.validate().then((valid) => {
          if (valid) {
            request(
              "/fisherApi/wechatMiniProgram/editShip",
              [{ ...formShipData.value }],
              "POST",
            )
              .then((res) => {
